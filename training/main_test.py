...
compression = None # hvd.Compression.fp16 if args.fp16_allreduce else hvd.Compression.none
...
def adjust_learning_rate(epoch, batch_idx, type="cosine"):
    if epoch < args.warmup_epochs:
        ...
        lr_adj = 1. / (epoch / args.warmup_epochs + 1)
    elif type == "linear":
        if epoch < 30:
            lr_adj = 1.
        elif epoch < 60:
...
class Metric(object):
    ...
    def update(self, val):
        self.sum += val.detach().cpu() # hvd.allreduce(val.detach().cpu(), name=self.name)
        self.n += 1
